...
@testset "SDF params/ParametricDerivative" begin
  ...
  # TODO (cg 2026/02/06 17:47): not sure why these tols are so bad.
  @test maximum(abs.(derivs[1] .- manual_derivs1)) < 1e-5
  @test maximum(abs.(derivs[2] .- manual_derivs2)) < 1e-5
  @test maximum(abs.(derivs[3] .- manual_derivs3)) < 1e-5
